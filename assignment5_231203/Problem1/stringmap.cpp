...
void StringMap::rehash(int nBuckets_new) {
   // TODO
   KeyValuePair *old = buckets;
   int nBuckets_old = nBuckets;

   buckets = new KeyValuePair[nBuckets_new];
   nBuckets = nBuckets_new;
   for (int i = 0; i < nBuckets; i++) buckets[i].occupied = false;
   for(int i = 0; i < nBuckets_old; i++)
   {
      if(old[i].occupied)
      {
         int pos = hashCode(old[i].key) % nBuckets;
         int cnt = 0;
         while(cnt < nBuckets)
         {
               if(!buckets[pos].occupied)
               {
                  ...
                  buckets[pos].key = old[i].key;
                  buckets[pos].value = old[i].value;
                  break;
               }
               pos = (pos + 1) % nBuckets;
               cnt++;
...
int StringMap::findKey(const string & key) const {
   ...
   while(cnt < nBuckets)
   {
      if(buckets[pos].key == key) return pos;
      pos = (pos + 1) % nBuckets;
      ++cnt;
   }
   return -1;
...
int StringMap::insertKey(const string & key) {
   ...
   pos = hashCode(key) % nBuckets;
   cnt = 0;
   while(cnt < nBuckets)
   {
      if(buckets[pos].occupied == false)
      {
         buckets[pos].occupied = true;
         buckets[pos].key = key;
         count++;
   ...
   error("insertKey: the Hash table is already full.");
}
